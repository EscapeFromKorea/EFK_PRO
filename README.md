...
### DreamThreadSystem — 꿈의 실타래 (Phase 1 + 2)
...
**Phase 2 — 세모 핀 벽 클라이밍.** 세모가 벽을 향해 **G**를 누르면 레이캐스트로 맞은 벽 표면에
고리(`ThreadAnchor`)를 런타임 생성하고 세모 자신도 그 자리에 **완전 고정 부착**합니다(`isKinematic`).
점프로 위로 도약하며 탈착 → 공중 이동 → 더 높은 벽에서 G 재부착, 이 루프로 벽을 타고 오릅니다
(수직 도약 직후 이동 방향이 벽과 평행이 되어 재부착이 실패하던 문제는 "마지막 부착 벽 방향" 폴백
레이로 해결). 고리는 동시 **2개**까지(3번째는 가장 오래된 것 자동 회수), 수동 회수는 전용 키 **T**
(박은 고리 전부 제거)입니다. 벽부착을 `mover.enabled=false` 대신 **`isKinematic`으로 구현해 로스터
churn을 아예 피했고**(컨트롤러 무변경), 부착 중엔 `PlayerJump`를 꺼 클라이밍 도약과 일반 점프의 충돌을
막습니다. 생성된 고리엔 Phase 1대로 구·세모가 F로 매달립니다(생성/벽부착만 세모 전용). 핀 배치·회수·
클라이밍은 별도 씬 싱글턴 `ThreadPinPlacer`가 전담하며, 자기·타 플레이어는 레이어가 아니라 계층
(`PlayerMover` 보유)으로 걸러 벽만 감지합니다 — PlayerSystem 파일은 여전히 수정하지 않습니다.

**매달림 중 접지 이동.** F로 매달린 채 바닥에 닿으면 접선 펌핑이 지면 그립 마찰에 짓밟혀 안 움직이므로,
접지 중엔 `FreezePositionX`(평면 고정)를 풀고 `PlayerMover`와 같은 규약(`inputYawOffset` 회전 + 전후좌우
2D 입력)으로 평소처럼 걸어 이동합니다(로프 조인트가 반경 L로 테더). 공중에선 매 프레임 다시 평면 고정 +
접선 펌핑으로 스윙합니다.
...
| `invertSwing` | `false` | 좌우 입력 방향이 의도와 반대로 느껴지면 켠다(옆모습 카메라 방향에 따라 다름). 접선 펌핑(공중)에만 적용 — 접지 걷기는 `inputYawOffset` 규약을 그대로 따른다. |
...
| `groundMoveSpeed` | `4f` | 매달린 채 바닥에 닿았을 때 걷는 속도(Unit/s). 접지 중엔 접선 펌핑이 그립 마찰에 막혀 안 움직이므로, `FreezePositionX`를 풀고 `PlayerMover`와 같은 규약(`inputYawOffset` + 전후좌우 2D)으로 velocity를 직접 대입해 평소처럼 걷는다(로프 반경 L 테더). 공중에선 다시 평면 고정 + 펌핑. |
...
#### ThreadPinPlacer.cs (Phase 2 — 세모 핀 벽 클라이밍)

씬에 하나 두는 싱글턴. `Tools > DreamThread > Create Pin Placer`로 생성. 세모 전용 G(박기+벽부착) /
점프(도약 탈착) / T(고리 전부 회수)를 처리한다.

| 필드 | 기본값 | 설명 |
|---|---|---|
| `rayRange` | `6f` | 세모 중심에서 이동 방향으로 쏴 벽을 찾는 레이 사거리(Unit). 이 안에 벽이 없으면 핀 생성 실패. |
| `wallLayer` | `~0`(Everything) | 핀을 박을 수 있는 '벽' 레이어. 자기·타 플레이어는 레이어가 아니라 계층(`PlayerMover` 보유)으로 걸러 제외하므로, 벽이 플레이어와 같은 Default 레이어여도 박힌다. 좁히고 싶을 때만 지정. |
| `pinSurfaceOffset` | `0.2f` | 핀을 벽 표면에서 법선 방향으로 띄우는 거리(Unit). 0이면 표면에 딱 붙고, 살짝 띄우면 실이 벽에 파묻히지 않아 걸기 쉽다. |
| `minMoveSpeed` | `0.5f` | velocity를 '이동 중'으로 인정하는 최소 수평 속력(Unit/s). 이보다 느리면 마지막 이동 방향(폴백)으로 레이를 쏜다. |
| `pinConnectRange` | `4f` | 생성된 핀 앵커의 연결 범위(`ThreadAnchor.connectRange`). 이 거리 안에서 F로 매달린다. |
| `pinMarkerSize` | `0.3f` | 핀 시각 마커(분홍 구) 지름(Unit). |
| `climbLeapSpeed` | `9f` | 점프로 탈착할 때 위로 주는 도약 속도(Unit/s). 부착은 그 자리에 완전 고정이고, 상승은 이 도약 → 공중 이동 → G 재부착 루프로만 한다. 도달 높이 ≈ v²/(2g)(대략 9면 ~4 Unit). |
...
| `airWallCheckDistance` | `0.7f` | 공중에서 벽으로 미는 입력 성분을 제거해, 지면 구르기용 고마찰 그립 PhysicMaterial이 몸을 벽에 붙들어 안 떨어지는 문제를 막는 벽 감지 거리(Unit). 몸 중심에서 이동 방향으로 이만큼 안에 벽(수직면)이 있으면 그 벽으로 미는 성분을 지워 벽면을 따라만 움직이고 중력으로 미끄러져 내려온다(레이어가 아니라 계층으로 자신·타 플레이어 제외). `0`이면 비활성. 접지 중엔 적용 안 함(정상 이동/구르기 유지). |
