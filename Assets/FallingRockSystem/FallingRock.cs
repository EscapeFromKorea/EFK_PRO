...
///
/// [소멸 지점은 권장상 SlowZone 볼륨보다 아래 — 2026-08-02부터 하드 제약 아님]
/// 감속 구역 안에서 Destroy되면 SlowZone.affected(Dictionary)에 죽은 참조가 남는다. SlowZone의
/// OnTriggerExit은 파괴되며 들어온 collider를 null 체크로 걸러내고 바로 반환하기 때문이다.
/// 다만 SlowZone이 이제 스스로 죽은 키를 주기적으로 걷어내므로(SlowZone.cs FixedUpdate 참고)
/// 최대 1초 정도 남았다 사라지는 수준이고 더 이상 영구 누적이 아니다 - 구역을 완전히 통과한 뒤
/// 소멸시키는 배치가 여전히 권장이지만(정리 주기 동안의 순회 비용이라도 줄이는 편이 낫다), 지키지
/// 못하는 배치가 사고로 번지지는 않는다. 위반 검사는 스포너(FallingRockSpawner.Validate)가 담당한다.
///
...
/// 발동시키는 우회가 생기지 않는다).
/// 파편이 감속 구역 안에서 소멸하면 위 [소멸 지점] 죽은 참조 문제가 파편 수만큼 배로 생기는데,
/// 바닥 충돌 지점은 설계상 구역 볼륨보다 아래이므로(despawnFallDistance 주석 참고) 파편은 구역에
/// 들어가지 않는다. 구역 안에 바닥을 두면 이 전제가 깨진다 — SlowZone이 주기적으로 정리하므로
/// 사고는 아니지만 여전히 권장하지 않는 배치다.
/// </summary>
...
public class FallingRock : MonoBehaviour
{
    ...
    [Tooltip("스폰 위치에서 이만큼 아래로 내려가면 소멸한다(절대 높이가 아니라 상대 거리 — " +
             "스폰 지점을 옮겨도 따라온다). 권장상 감속 구역(SlowZone) 볼륨보다 아래에서 소멸시켜라 " +
             "— SlowZone이 죽은 참조를 주기적으로 청소하므로(2026-08-02) 지키지 않아도 사고로 " +
             "번지지 않지만, 구역 안에서 소멸하면 정리될 때까지 순회 비용이 남는다. 기본 8은 " +
             "'6칸 구역 상단에서 스폰 → 구역 하단(=통과 통로)까지 6칸 낙하 → 구역 바닥보다 2칸 더 " +
             "아래에서 소멸' 배치를 전제한 값이다.")]
    ...
    private void FixedUpdate()
    {
        ...
        if (Time.time - movingSince >= stuckTimeout)
        {
            Debug.LogWarning($"[FallingRock] '{name}'이 낙하 도중 {stuckTimeout}초간 멈춰 있어 " +
                             ...
                             "잠깐 남는다(주기적으로 정리됨) — 권장상 스폰 지점 아래 낙하 경로(구역 " +
                             "바닥 아래까지)를 비워라.", this);
            Destroy(gameObject);
        }
    }
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        // 도형용으로 남는다).
        //
        // 파편이 감속 구역 안에서 소멸하면 위 [소멸 지점] 죽은 참조가 파편 수만큼 생긴다(SlowZone이
        // 주기적으로 정리하므로 영구는 아니다) — 통과 통로는 설계상 구역 밑이라 괜찮지만, 구역
        // <b>안</b>에서 맞으면 파편 수만큼 남는다. 실제로 문제가 되면 shardCount를 0으로 두는 것이
        // 아니라 통로 배치를 본다.
        Shatter(collision.relativeVelocity.magnitude);
    }
