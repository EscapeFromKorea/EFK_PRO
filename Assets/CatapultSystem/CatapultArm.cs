...
public class CatapultArm : MonoBehaviour
{
    ...
    [Tooltip("당김 비율 1(완전 장전)일 때의 로컬 X 각도. rest보다 90°에 가까워질수록 버킷이 낮아진다. " +
             ...
             "올렸다** — 재검산 결과 θ≈92.36°에서야 파묻히기 시작해 90°는 그보다 작지만(마진 " +
             "+0.168, 45°의 +3.380보다 훨씬 얇다) 여전히 파묻히지 않는다(클래스 상단 주석 재검산 " +
             "참고). 3배 균등 확대(6차 3단계)는 각도를 바꾸지 않는다 — 길이만 스케일된다.")]
    public float pulledAngle = 90f;
