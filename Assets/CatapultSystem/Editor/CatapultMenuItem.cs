...
public static class CatapultMenuItem
{
    ...
    // 아래 BucketGroupLocalY 등 별도 상수) — 클래스 상단 주석의 파묻힘 검산이 이 값들을 그대로
    // 쓴다. 값을 바꾸면 그 검산도 다시 해야 한다. 정육면체(1×1×1) 기준 확실한 여유로 설계됐다:
    // X 여유 0.4(각 옆면), Y 여유 0.2(위아래 각각), Z 여유 0.35(각 옆면) — pre-scale 기준.
    ...
    private static void CreateCatapult()
    {
        ...
        GameObject bucketInner = CreateBucket(armPivot.transform, out GameObject anchor); // 앵커도 여기서 생성(버킷 그룹에 부모화).
    ...
    private static GameObject CreateBucket(Transform parent, out GameObject anchor)
    {
        ...
        // 벽 3면(B/L/R): 바닥 윗면(-half.y)부터 위로 BucketWallHeight만큼. F(그룹 로컬 +Z)만
        // 제거됐다(클래스 상단 "F 벽 제거" 주석의 좌표 유도 참고).
    ...
    private static GameObject CreateSteerRingVisual(Transform parent, Vector3 localCenter, Material mat)
    {
        ...
        float segmentLength = (2f * Mathf.PI * SteerRingRadius / SteerRingSegmentCount) * 1.15f; // 세그먼트 사이 틈 방지 여유.
        ...
        for (int i = 0; i < SteerRingSegmentCount; i++)
        {
            ...
            // (SteerRingTubeThickness)를 줘야 세그먼트들이 원둘레를 따라 이어 붙어 실제 닫힌
            // 원으로 보인다(반대로 주면 중심에서 바깥으로 뻗는 꽃잎/바큇살처럼 보인다).
